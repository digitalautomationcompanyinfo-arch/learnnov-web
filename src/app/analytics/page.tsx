...
import { loadDatabase, DBStore } from '@/services/db-store';
...
export default function AnalyticsPage() {
  ...
  const completedCoursesCount = db?.certificates.length || 2;
  const examsPassedCount = db?.exams.length || 3;
  ...
  return (
    <main className="dashboard-container" dir={isRtl ? 'rtl' : 'ltr'}>
      {/* Header Banner */}
      <div className="glass-panel profile-header" style={{ borderLeft: '5px solid #06B6D4', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1.5rem' }}>
        <div>
        ...
        <button onClick={handleDownloadPdf} style={{ background: 'linear-gradient(135deg, #10B981, #06B6D4)', color: '#FFF', border: 'none', padding: '0.75rem 1.5rem', borderRadius: '12px', fontWeight: 800, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1.25rem', marginTop: '2rem' }}>
        <div className="glass-panel" style={{ padding: '1.25rem', borderRadius: '14px', borderLeft: '4px solid #6366F1' }}>
          <div style={{ fontSize: '0.85rem', color: '#94A3B8' }}>{language === 'ar' ? 'ساعات التعلم التراكمية' : 'Total Study Hours'}</div>
          <div style={{ fontSize: '1.8rem', fontWeight: 900, color: '#FFF', margin: '0.4rem 0' }}>{totalLearningHours} <span style={{ fontSize: '1rem', fontWeight: 500 }}>ساعة</span></div>
          <div style={{ fontSize: '0.75rem', color: '#10B981' }}>📈 +18% {language === 'ar' ? 'مقارنة بالأسبوع الماضي' : 'vs last week'}</div>
        </div>

        <div className="glass-panel" style={{ padding: '1.25rem', borderRadius: '14px', borderLeft: '4px solid #06B6D4' }}>
          <div style={{ fontSize: '0.85rem', color: '#94A3B8' }}>{language === 'ar' ? 'أيام الاستمرارية المباشرة' : 'Current Study Streak'}</div>
          <div style={{ fontSize: '1.8rem', fontWeight: 900, color: '#FFF', margin: '0.4rem 0' }}>{streakDays} <span style={{ fontSize: '1rem', fontWeight: 500 }}>يوم متوالي 🔥</span></div>
          <div style={{ fontSize: '0.75rem', color: '#06B6D4' }}>🔥 {language === 'ar' ? 'أفضل تتابع تعليمي لك هذا الشهر' : 'Best monthly streak'}</div>
        </div>

        <div className="glass-panel" style={{ padding: '1.25rem', borderRadius: '14px', borderLeft: '4px solid #10B981' }}>
          <div style={{ fontSize: '0.85rem', color: '#94A3B8' }}>{language === 'ar' ? 'معدل النجاح والاختبارات' : 'Exam Success Rate'}</div>
          <div style={{ fontSize: '1.8rem', fontWeight: 900, color: '#FFF', margin: '0.4rem 0' }}>96.5%</div>
          <div style={{ fontSize: '0.75rem', color: '#10B981' }}>🎯 {examsPassedCount} {language === 'ar' ? 'اختبارات مجتازة بنجاح' : 'exams passed'}</div>
        </div>

        <div className="glass-panel" style={{ padding: '1.25rem', borderRadius: '14px', borderLeft: '4px solid #F59E0B' }}>
          <div style={{ fontSize: '0.85rem', color: '#94A3B8' }}>{language === 'ar' ? 'الترتيب بين الدفعة' : 'Cohort Percentile'}</div>
          <div style={{ fontSize: '1.8rem', fontWeight: 900, color: '#FFF', margin: '0.4rem 0' }}>Top 3%</div>
          <div style={{ fontSize: '0.75rem', color: '#F59E0B' }}>🏆 {language === 'ar' ? 'ضمن قائمة المتفوقين' : 'Honor Roll Student'}</div>
        </div>
      </div>
