...
import { loadDatabase, saveDatabase, DBStore } from '@/services/db-store';
...
export default function AssignmentsPage() {
  ...
  const handleSubmitAssignment = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      // Audit log in DB store
      const loadedDb = loadDatabase();
      loadedDb.auditLogs.unshift({
        id: Date.now(),
        user: userName || 'طالب ليرنوف',
        action: 'تسليم مشروع تطبيقي',
        resource: `Assignment: ${selectedAssignment.title}`,
        ip: '197.245.89.12',
